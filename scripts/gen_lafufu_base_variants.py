...
try:
    ...
    raise SystemExit("Pillow required: uv pip install pillow") from None
...
def build_graph(ref_name: str, seed: int, denoise: float, pos: str, neg: str) -> dict:
    return {
        ...
        "10": {
            ...
            "inputs": {"images": ["9", 0], "filename_prefix": f"lafufu_base_{seed}"},
        },
    }


def run_one(
    ref_name: str, seed: int, denoise: float, pos: str, neg: str, out_dir: Path, idx: int
) -> Path:
...
def main() -> None:
    ...
    ap.add_argument(
        "--denoise", type=float, default=None, help="override default denoise for this mode"
    )
    args = ap.parse_args()
